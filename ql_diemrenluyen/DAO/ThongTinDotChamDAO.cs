    ...
    public class ThongTinDotChamDAO
    {
        ...
        public static bool AddThongTinDotCham(ThongTinDotChamDTO thongTin)
        {
            string sql = "INSERT INTO thongtindotchamdiem (dotchamdiem_id, covan_id, sinhvien_id, khoa_id, final, hoanthanh, status) " +
                         "VALUES (@dotChamDiemId, @coVanId, @sinhVienId, @khoaId, @final, @hoanThanh, @status)";
        ...
        public static bool UpdateThongTinDotCham(int dotChamDiemId, string role, long sinhVienId, long nguoiChamId)
        {
            ...
            if (role == "Sinh Viên")
            {
                sql = "UPDATE thongtindotchamdiem SET hoanthanh = 1 WHERE dotchamdiem_id = @dotChamDiemId AND sinhvien_id = @sinhVienId AND sinhvien_id = @nguoiChamId";
            ...
            {
                sql = "UPDATE thongtindotchamdiem SET covan_id = @nguoiChamId, hoanthanh = 1 WHERE dotchamdiem_id = @dotChamDiemId AND sinhvien_id = @sinhVienId";
                cmd.Parameters.AddWithValue("@dotChamDiemId", dotChamDiemId);
                cmd.Parameters.AddWithValue("@nguoiChamId", nguoiChamId);
                cmd.Parameters.AddWithValue("@sinhVienId", sinhVienId);
            }
            ...
            {
                sql = "UPDATE thongtindotchamdiem SET khoa_id = @nguoiChamId, hoanthanh = 1 WHERE dotchamdiem_id = @dotChamDiemId AND sinhvien_id = @sinhVienId";
                cmd.Parameters.AddWithValue("@dotChamDiemId", dotChamDiemId);
                cmd.Parameters.AddWithValue("@nguoiChamId", nguoiChamId);
                cmd.Parameters.AddWithValue("@sinhVienId", sinhVienId);
            }
            ...
            {
                sql = "UPDATE thongtindotchamdiem SET final = 1, hoanthanh = 1 WHERE dotchamdiem_id = @dotChamDiemId AND sinhvien_id = @sinhVienId";
                cmd.Parameters.AddWithValue("@dotChamDiemId", dotChamDiemId);
                cmd.Parameters.AddWithValue("@sinhVienId", sinhVienId);
            }
            else
