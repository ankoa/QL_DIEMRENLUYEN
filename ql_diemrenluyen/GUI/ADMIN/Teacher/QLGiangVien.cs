...
namespace ql_diemrenluyen.GUI.ADMIN
{
    public partial class QLGiangVien : Form
    {
        ...
        private void LoadGV()
        {
            try
            {
                // Assuming GiangVienBUS.GetAllGiangVien() returns a list of GiangVienDTO
                List<GiangVienDTO> lecturers = GiangVienBUS.GetAllGiangVien();
                if (lecturers == null || lecturers.Count == 0)
                {
                    MessageBox.Show("Không có giảng viên nào để hiển thị.");
                    return;
                }
                ...
                {
                    // Map ChucVu to a descriptive role name (e.g., using a helper method)
                    tableTK.Rows.Add(
                        lecturer.Id,
                        lecturer.Name, 
                        lecturer.Email,
                        lecturer.NgaySinh.HasValue ? lecturer.NgaySinh.Value.ToString("dd/MM/yyyy") : "",
                        lecturer.GioiTinh==1? "Nam" : "Nữ",
                        KhoaBUS.GetKhoaByID(lecturer.KhoaId),
                        lecturer.CreatedAt.HasValue ? lecturer.CreatedAt.Value.ToString("dd/MM/yyyy") : "",
                        lecturer.UpdatedAt.HasValue ? lecturer.UpdatedAt.Value.ToString("dd/MM/yyyy") : "",
                        lecturer.Status == 1 ? "Hoạt động" : "Không hoạt động"
                    );
                }

                ApplyTableStyles(); // Apply styles to the table
            }
            catch (Exception ex)
            {
        ...
        // Example helper method for mapping role IDs to role names
